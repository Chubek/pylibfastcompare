...
void init_hmn(hmn_t *self, hmsize_t index, seq_t seq, hmsize_t len);
void resize_insert_hmv(hmvalue_s *self, uint64_t *seq);
void resize_hashmap(hm_s *self);
void free_hashmap_vec(hmvalue_s self);
void free_hashmap(hm_s *self);
void insert_into_hashmap(hm_s *self, uint64_t key, uint64_t *value, hmsize_t len_seq);
